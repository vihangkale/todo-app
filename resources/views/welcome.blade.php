...
<div class="w-100 h-100 d-flex justify-content-center align-items-center">
	<div class="w-50">
		...
		<h2 class="text-white">What's your next task? Add something </h2>
		<form action="{{route('todo.store')}}" method="POST">
		@csrf <!--protect your application from cross-site request forgery (CSRF) attacks-->
		<div class="input-group mb-3 w-100">
			<input type="text" class="form-control form-control-lg" name="title" placeholder="Type here" aria-label="Recipient's username" aria-description="button-addon2">
...
@else
  ...
  <path d="M5 12l5 5l10 -10" />
	...
	@endif{{$todo->title}}</div>
				...
				@method("PUT")
				@csrf
				<input type="text" name="completed" value="1" hidden> <!-- input for backend database-->
		...
		@method("PUT")
		@csrf
		<input type="text" name="completed" value="0" hidden> <!-- input for backend database-->
		<button class="btn btn-warning">Mark it as Incompleted</button>
		</form>
		@endif
		<a class="inlane block" href="{{route("todo.edit",$todo->id)}}">
			<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-edit ml-4" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="#c14368" fill="none" stroke-linecap="round" stroke-linejoin="round">
  			<path stroke="none" d="M0 0h24v24H0z" fill="none"/>
